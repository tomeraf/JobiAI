...
def create_mock_client():
    """Create a mock LinkedInClient with default behaviors."""
    mock_client = MagicMock()
    mock_client.check_abort = MagicMock()
    mock_client.clear_abort = MagicMock()
    mock_client.set_current_job = MagicMock()
    mock_client.check_for_replies = AsyncMock(return_value={"replied_contacts": [], "failed_contacts": []})
    mock_client.search_company_all_degrees = AsyncMock(return_value={
        "first_degree": [],
        "second_degree": [],
        "third_plus": [],
        "messages_sent": []
    ...
    mock_client.send_connection_request = AsyncMock(return_value=True)
    return mock_client


class TestWorkflowWithMockedLinkedIn:
    """Test workflow with mocked LinkedIn client."""

    @pytest.fixture
    def mock_linkedin_client(self):
        """Mock LinkedInClient singleton."""
        with patch("app.services.workflow_orchestrator.LinkedInClient") as MockClient:
    ...
    async def test_workflow_with_first_degree_connections(
        ...
        english_template: Template, mock_linkedin_client
    ):
        """Workflow should message 1st degree connections."""
        english_template.is_default = True
        await db_session.flush()

        mock_linkedin_client.search_company_all_degrees = AsyncMock(return_value={
            "first_degree": [
...
class TestHebrewNameWorkflowPause:
    ...
    @pytest.fixture
    def mock_linkedin_client(self):
        """Mock LinkedInClient for Hebrew name tests."""
        with patch("app.services.workflow_orchestrator.LinkedInClient") as MockClient:
            mock_client = create_mock_client()
            MockClient.get_instance = MagicMock(return_value=mock_client)
            yield mock_client
    ...
    async def test_workflow_pauses_for_missing_hebrew_name(
        self, db_session: AsyncSession, job_with_company: Job,
        default_template: Template, mock_linkedin_client
    ):
        """Workflow should pause when Hebrew translation is missing."""
        mock_linkedin_client.search_company_all_degrees = AsyncMock(
    ...
    async def test_workflow_resumes_after_hebrew_names_provided(
        self, db_session: AsyncSession, default_template: Template, mock_linkedin_client
    ):
...
class TestReplyChecking:
    ...
    @pytest.fixture
    def mock_linkedin_client(self):
        """Mock LinkedInClient for reply checking tests."""
        with patch("app.services.workflow_orchestrator.LinkedInClient") as MockClient:
            mock_client = create_mock_client()
            MockClient.get_instance = MagicMock(return_value=mock_client)
            yield mock_client
    ...
    async def test_reply_detected_returns_reply_step(
        ...
        default_template: Template, messaged_contacts, mock_linkedin_client
    ):
        """Workflow should return reply_received step when reply detected."""
        mock_linkedin_client.check_for_replies = AsyncMock(return_value={
            "replied_contacts": [{"name": "John Doe"}],
            "failed_contacts": []
        })
    ...
    async def test_force_search_searches_for_new_people(
        ...
        assert result["success"] is True
        mock_linkedin_client.search_company_all_degrees.assert_called_once()
...
class TestContactSaving:
    ...
    @pytest.fixture
    def mock_linkedin_client(self):
        """Mock LinkedInClient for contact tests."""
        with patch("app.services.workflow_orchestrator.LinkedInClient") as MockClient:
            mock_client = create_mock_client()
            MockClient.get_instance = MagicMock(return_value=mock_client)
            yield mock_client
...
class TestActivityLogging:
    ...
    @pytest.fixture
    def mock_linkedin_client(self):
        """Mock LinkedInClient for logging tests."""
        with patch("app.services.workflow_orchestrator.LinkedInClient") as MockClient:
            mock_client = create_mock_client()
            MockClient.get_instance = MagicMock(return_value=mock_client)
            yield mock_client
